...
library(Hmisc)
tekelp <- read_csv("../Data/Annual_Kelp_All_Years_20250903.csv")
...
One analysis that we talked about this quarter that I could apply to answer my research: "Is there a significant difference in mean sleep duration between the first half and second half of the work week?" is a students two sample t-test. This analysis is appropriate with regard to my variables because in my study I address two variables, the half of the work week and sleep duration. The half of the work week is my predictor variable, which is a categorical variable split into two independant parts; the first half of the week and the second half of the week. Comparatively my response variable, Sleep duration (hrs), is a continuous variable. 
...
sleep_calendar_clean <- sleep_calendar |> 
  clean_names() |> 
  select(duration_hr_mm_ss, half_of_week)

```

Assumptions:
1. Continuous variable: Our response variable, sleep duration, is continuous because it’s measured in hours and can take any numeric value, not just fixed categories.

2. Independent samples: Our predictor variable, the half of the work week, has two independant groups because each night’s sleep measurement belongs to either the first half of the week (Monday/Tuesday) or the second half (Wednesday/Thursday), but not both. No single observation appears in both groups, so the sleep durations in one group do not influence or overlap with the other group.
...
ggplot(data = sleep_calendar_clean |> 
       ...
       aes(sample = duration_hr, # use numeric hours for QQ plot
           color = half_of_week)) + # color points by half_of_week
  geom_qq_line(color = "blue") + # add reference line in blue
  geom_qq() + # add actual QQ plot points
  facet_wrap(~ factor(half_of_week, 
                      levels = c(1,2), 
...
var.test(duration_hr_mm_ss ~ half_of_week, # run variance test between first half and second half of the week
         data = sleep_calendar_clean %>% filter(half_of_week != 3)) # exclude half 3
```
...
t.test(as.numeric(duration_hr_mm_ss) ~ half_of_week,  # run a two-sample t-test
       ...
       var.equal = TRUE)  # assume equal variances based on F-test
```
Effect Size:

```{r}
sleep_two_groups <- sleep_calendar_clean |>  
  filter(half_of_week != 3) |> # keep only first half (1 = Mon/Tues) and second half (2 = Wed/Thurs)
  mutate(duration_hr = as.numeric(duration_hr_mm_ss)/3600,  # convert hms object to numeric hours
         half_of_week = factor(half_of_week, levels = c(1,2),  # convert grouping variable to factor
                               labels = c("Mon/Tues", "Wed/Thurs")))  
...
ggplot(data = sleep_calendar %>% filter(half_of_week != 3),
